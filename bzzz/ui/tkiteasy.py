...
class Canevas(tk.Canvas):
    def __init__(self, parent: tk.Misc, largeur: float, hauteur: float) -> None:
        ...
        # Attributs
        self.images: dict[
            tuple[str | PathLike[str], int | None, int | None],
            ImageTk.PhotoImage,
        ...
        self.attendre_clic = tk.BooleanVar(value=False)
        self.attendre_touche = tk.BooleanVar(value=False)
    ...
    def afficherTexte(
        ...
        font = Font(family="Helvetica", size=taille, weight=weight)
        return ObjetGraphique(
            self.create_text(
                x, y, fill=couleur, text=texte, font=font, anchor=ancre, tags=tags
            ),
            x,
            y,
            couleur,
    ...
    def dessinerRectangle(
        ...
        return ObjetGraphique(
            self.create_rectangle(
                x, y, x + largeur, y + hauteur, fill=couleur, width=0, tags=tags
            ),
            x,
            y,
            couleur,
    ...
    def dessinerLigne(
        ...
        return ObjetGraphique(
            self.create_line(x, y, x2, y2, fill=couleur, tags=tags), x, y, couleur
        )
    ...
    def afficherImage(
        ...
        Args:
            ...
            width (int | None): La largeur en pixels de l'image qui sera affichée
            height (int | None): La hauteur en pixels de l'image qui sera affichée
        ...
        image_key = (fichier, width, height)

        if image_key in self.images:
            photo_image = self.images[image_key]
        else:
            img: Image.Image | ImageFile.ImageFile = Image.open(fichier)

            if width is not None and height is not None:
                img = img.resize((width, height), Image.Resampling.LANCZOS)

            photo_image = ImageTk.PhotoImage(img)
            self.images[image_key] = photo_image

        return ObjetGraphique(
            self.create_image(x, y, image=photo_image, anchor=ancre, tags=tags),
            x,
            y,
            "",
        )
    ...
    def attendreTouche(self) -> str:
        ...
        """
        self.attendre_touche.set(False)
        self.wait_variable(self.attendre_touche)
        touche = self.recupererTouche()

        assert touche is not None
    ...
    def attendreClic(self) -> "tk.Event[Canevas]":
        ...
        Examples:
            ...
            >>> print(f"x = {clic.x} | y = {clic.y}")
        """
        self.attendre_clic.set(False)
        self.wait_variable(self.attendre_clic)
